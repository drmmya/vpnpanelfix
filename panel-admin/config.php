...
if (empty($_SESSION['csrf_token'])) {
  ...
  catch (Throwable $e) { $_SESSION['csrf_token'] = bin2hex(openssl_random_pseudo_bytes(32)); }
}
...
function csrf_field(){ return '<input type="hidden" name="csrf_token" value="'.esc(csrf_token()).'">'; }
function csrf_check(){ return isset($_POST['csrf_token']) && hash_equals(csrf_token(), (string)$_POST['csrf_token']); }
function require_csrf(){ if($_SERVER['REQUEST_METHOD']==='POST' && !csrf_check()){ http_response_code(403); exit('Invalid security token. Refresh and try again.'); } }
